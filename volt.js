...
client.on('messageCreate', async message => {
  ...
  if (command === 'بان') {
    ...
    if (!member.bannable)
      return message.reply('❌ ما أقدر أبند هذا الشخص.');
  ...
  if (command === 'فكبان') {
    ...
    if (!userId)
      return message.reply('❌ حط آيدي الشخص.');

    try {
      await message.guild.members.unban(userId);

      return message.reply(
        `✅ تم فك البان عن <@${userId}>.`
      );
    } catch {
      return message.reply(
        '❌ ما لقيت هذا الشخص ضمن المبندين.'
      );
    }
  }
  ...
  if (command === 'طرد') {
    ...
    if (!member.kickable)
      return message.reply('❌ ما أقدر أطرد هذا الشخص.');
  ...
  if (command === 'تايم') {
    ...
    if (!days || days < 1 || days > 28)
      return message.reply(
        '❌ حدد المدة بالأيام من 1 إلى 28.'
      );

    if (!member.moderatable)
      return message.reply(
        '❌ ما أقدر أعطيه تايم أوت.'
      );
  ...
  if (command === 'فكتايم') {
    ...
    return message.reply(
      `✅ تم فك التايم عن ${member.user.tag}.`
    );
  }
  ...
  if (command === 'رتبة') {
    ...
    if (
      ...
      return message.reply(
        '❌ ما أقدر أعطي رتبة أعلى من رتبتي.'
      );
